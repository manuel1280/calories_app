module RenderCaloriesHelper
  ...
  def calories_data
    ...
    calories=ActiveRecord::Base.connection.execute("SELECT created_at, SUM(value) AS 'total_value' FROM calories WHERE user_id = #{user} and created_at > '#{time_ago}' GROUP BY created_at;")
    calories_won=ActiveRecord::Base.connection.execute("SELECT created_at, SUM(value) AS 'total_won' FROM calories WHERE user_id = #{user} and created_at > '#{time_ago}' and type_value = 'won' GROUP BY created_at;")
    calories_lost=ActiveRecord::Base.connection.execute("SELECT created_at, SUM(value) AS 'total_lost' FROM calories WHERE user_id = #{user} and created_at > '#{time_ago}' and type_value = 'lost' GROUP BY created_at;")
    show_calories=[]
